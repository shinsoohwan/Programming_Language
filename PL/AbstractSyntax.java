...
class Declarations extends ArrayList<Declaration> {
	...
	public void display(int k){
		...
		int i;
    	for(i=0;i<k;i++) System.out.print("\t");
		System.out.println("Declarations : ");

		for(i=0;i<this.size();i++) this.get(i).display(k); 
		System.out.println();
	}
	public void output(int k) throws IOException {
		for(int i=0;i<this.size();i++) this.get(i).output(k);
	}
}
...
// Declaration = Variable v; Type t
    ...
    public void output(int k) throws IOException{
    	...
    	BufferedWriter bw=new BufferedWriter(new FileWriter("./Soez.java",true));
    	...
    	if(t.equals(Type.FLOAT)) pw.println("double "+v+";");
    	else if(t.equals(Type.BOOL)) pw.println("boolean "+v+";");
    	else pw.println(t+" "+v+";");
    	pw.flush();
    	pw.close();
    }
...
abstract class Statement {
    // Statement = Skip | Block | Assignment | Conditional | Loop | Print | Scan
	public void display(int k) {
...
class Block extends Statement {
    ...
    public void display(int k) {
    	...
    	for(i=0;i<this.members.size();i++) this.members.get(i).display(k);
    	for(i=0;i<k;i++) System.out.print("\t");
    }
    public void output(int k) throws IOException{
    	BufferedWriter bw=new BufferedWriter(new FileWriter("./Soez.java",true));
    	...
    	blockcount++;
    	if(blockcount!=1) pw.println("{");
    	pw.flush();
    	i=0;
    	while(i<this.members.size()) {
    		this.members.get(i).output(k);
    		i++;
    	}
    	i=0;
      	while(i<k-1) {
...
class Scan extends Statement{
	...
	public void output(int k) throws IOException {
		...
		if(count==1) 
			{
			for(i=0;i<k;i++) pw.write("\t");
			pw.println("Scanner scan=new Scanner(System.in);");
			}
		...
		switch(t.toString())
		{
		case "int":pw.write(v+"=scan.nextInt();"); pw.flush(); break;
		case "char":
			pw.println("try{"+v+"=(char)System.in.read();}");
			pw.println("\tcatch (IOException e) { System.out.println(e); System.exit(0);}");
			pw.flush(); break;
		case "float":pw.write(v+"=scan.nextDouble();"); pw.flush(); break;
		}
		pw.println();
		pw.flush();
	}
...
// Binary = Operator op; Expression term1, term2
    ...
    public void output(int k) throws IOException {
    	...
    	boolean b1=term1 instanceof Binary;
    	boolean b2=term2 instanceof Binary;
